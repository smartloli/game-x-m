...
DIALUP_PID=$GAME_X_HOME/bin/storm_stats_pid.pid
start()
{
	...
	CLASS=org.smartloli.game.x.m.book_12.server.KafkaTopology
...
stop()
{
	 ...
	 if [ -f $GAME_X_HOME/bin/storm_stats_pid.pid ];then
                    SPID=`cat $GAME_X_HOME/bin/storm_stats_pid.pid`
					  if [ "$SPID" != "" ];then
						 kill -9  $SPID
